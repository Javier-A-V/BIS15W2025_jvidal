...
```{r}
filter(fish, lakeid == "AL") #one = sign will not work because it is checking in both directions 
```
...
```{r}
filter(fish, length %in% c(167, 175)) #multiple observations, using the in operator, filtering in fish in the variable length, values of 167 and 175

#filtering for values within the same variable 
```

Alternatively, you can use `between` if you are looking for a range of specific values.  
```{r}
scalelength <- filter(fish, between(scalelength, 2.5, 3))
```
...
```{r}
filter(fish, scalelength %in% c(2, 4)) #this does not work to find ranges between 2 and 4 because the function is searching for scalelength that is exactly 2 and 4
```
