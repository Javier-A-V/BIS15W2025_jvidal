...
```{r}
fish%>% #taking fish and piping it into the select function, specifies the df  
  select(lakeid, scalelength) %>% #piped into select
  filter(lakeid=="AL") #piped into filter . order matters 
```
...
fish %>% #specify the fish data 
  select(lakeid, radii_length_mm) %>% #piped into select to pull out variables 
  filter(lakeid=="AL" | lakeid=="AR") %>% #would not be able to find a lakeid that is both AL and AR at the same time so use | not &
  filter(between(radii_length_mm, 2, 4)) %>% #extract between 2 and 4 
  arrange(-radii_length_mm) #sort in descending order
...
fish %>% 
  ...
  arrange(scalelength) #specify the variable to be sorted 
```
...
fish %>% 
  select(lakeid, scalelength) %>% 
  arrange(desc(scalelength)) #could descend like this or with - sign 
```
...
```{r}
mammals %>% 
  select(family, gestation, Genus) %>% 
  filter(family=="Felidae") %>% 
  filter(gestation<3 & gestation!="-999") %>% 
  arrange(-gestation)
```
...
fish %>% 
  ...
  select(lakeid, scalelength, scalelength_cm) #mutate and create new column from existing one 
```
...
```{r}
mammals %>% 
  filter(`wean mass`!="-999" & gestation!="-999" & mass!="-999") %>% #do this first so -999 is not included in calculation 
  mutate(mass_kg=mass/1000) %>% 
  mutate(mean_gestation_ratio=`wean mass`/gestation) %>% 
