...
```{r}
inner_join(table_A, table_B, by="customer_ID") #only going to join things that are exact matches 
```
...
`full_join`
```{r}
full_join(table_A, table_B, by="customer_ID") #joins everything as long as you have a unique identifier 
```

`anti_join`
Provides the rows in the first table for which there are not matching values in the second table.
```{r}
anti_join(table_A, table_B, by="customer_ID") #finding observations that are problems 
```
